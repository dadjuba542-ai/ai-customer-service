# 功能开关（案例系统 / 音频课程 / 人工客服系统）

三套系统各有独立开关，互不影响。所有开关集中登记在 `services/feature_flags.py` 的
...
| `audio_courses` | 音频课程：独立问答入口、标签/全文检索、后台课程管理、音频上传与外链 | `audio_courses_enabled` | `AUDIO_COURSES_ENABLED` | **关闭** |
...
1. **HTTP 入口**：`install_request_guard(app)` 注册的 `before_request` 按注册表里的路径前缀拦截
   （接口返回 `403 + code=feature_disabled`，页面返回说明页）。`cases` / `audio_courses` /
   `handoff` / `admin_handoff` 四个 blueprint 各带一层 `before_request` 兜底，防止前缀漏配。
...
`scripts/test_feature_flags.py` 覆盖默认状态、各系统独立开关、入口/调用链/定时任务禁用、
批量更新、未知开关 404；`scripts/test_audio_courses.py` 覆盖音频课程的三层拦截。
